...
- ~image_io_load~ is used to load either type of ~svg~ file (~image-io~ or ~svg~).
- For images of type ~svg~, the function ~svg_load~ is registered if =RSVG= support is compiled in.
- There is also a generic ~native-image~ type, which also just displays via ~image-io~.
...
- If and when we land in ~image-io~ (e.g. because we don't have =RSVG= and its ~svg_load~, or because ~image-mode~ has created an explicity ~:imagemagick~ type image):
  ...
  2. It adds a style sheet (with CSS!) to update the foreground color and draw a background rectangle behind it, and then calls ~mac_document_create_with_data~ or ~mac_document_create_with_url~.
  3. The latter two look through the list of possible "document types":

     * ~EmacsPDFDocument.class~
     * ~EmacsSVGDocument.class~
     * ~EmacsDocumentRasterizer.class~

     attempting to create a class of type ~EmacsDocumentRasterizer~ (using either the ~URL~, aka filename, or ~Data~ of the SVG file), and taking the first one that returns a valid document (running it in the main GUI thread if necessary).
- Meanwhile, back in ~EmacsSVGDocument~, we check if we have =USE_WK_API= set, and if so, we create a ~WKWebView~ (vs. a plain ~WebView~).
- In stock emacs-mac, we /do/ have =USE_WK_API= set.  It used to test:
  ...
  (since ~WK_API_ENABLED~ was removed as a compiler variable in MacOS v10.15).
...
3. If =RSVG= is /not available/, =UTI_SVG= is returned from ~mac_get_uti_from_image_type~, indicating that emacs-mac should display them natively, using the ~image-io~ webkit-svg chicanery above.
